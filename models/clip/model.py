...
class ResidualAttentionBlock_MaPLe(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0, add_adapter=False):
        super().__init__()
        ...
        if add_adapter:
            self.adapter = AdaptLayer(d_model=d_model)
        else:
            self.adapter = None

        # ############ prompt gated
        if (i > 0) and (i < self.prompt_depth) and self.gate:
            self.gamma = nn.Parameter(torch.ones(self.compound_prompt_nctx, 1024) * 1e-6)
        else:
            self.gamma = None
        # ############
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
                 text_layer=False, design_details=None, add_adapter=None):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i, add_adapter[i])
                  for i in range(layers)])
        else:
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
        ...
        ########### vit adapter num ############
        num_vit_adapter = design_details['vit_adapter_list']
        add_adapter = [False] * layers
        if len(num_vit_adapter) > 0:
            for i in num_vit_adapter:
                add_adapter[i] = True
        ########################################
        ...
        self.transformer = Transformer(width, layers, heads, design_details=design_details, add_adapter=add_adapter)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        ########### text adapter num ############
        num_vit_adapter = design_details['text_adapter_list']
        add_adapter = [False] * transformer_layers
